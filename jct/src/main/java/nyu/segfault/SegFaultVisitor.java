...
import nyu.segfault.VisitExpressionStatements;
...
//		method_only_VT.add(root.getString(3));
		...
		new Visitor(){
			...
			/* Determine and print the declarator type. */
				new VisitFieldDeclarations(impWriter, headWriter, constructorProp).dispatch(n);
            }
			...
			public void visitReturnStatement(GNode n) {
				...
				new VisitReturnStatements(impWriter, headWriter).dispatch(n);
			}

            public void visitExpressionStatement(GNode n) {
            	new VisitExpressionStatements(impWriter, headWriter).dispatch(n);
            }
